...
namespace AarquieSolutions.NodeGraph.Editor {
    /// <summary>
    /// This asset processor resolves an issue with the new v2 AssetDatabase system present on 2019.3 and later. When
    /// renaming a <see cref="NodeGraph.NodeGraph"/> asset, it appears that sometimes the v2 AssetDatabase will swap which asset
    /// is the main asset (present at top level) between the <see cref="NodeGraph.NodeGraph"/> and one of its <see cref="NodeGraph.Node"/>
    /// sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    /// finds a case where a <see cref="NodeGraph.Node"/> has been made the main asset it will swap it back to being a sub-asset
